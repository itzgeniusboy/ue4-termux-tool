...
The neon primary UI now presents one **PakForge Control Center**. It is the single normal place to start guided PAK work, inspect PAK metadata, unpack with optional Lua decompilation, repack with verification, run the Lua pipeline, run the auto pipeline, view setup status/logs, and access diagnostics. The menu uses simple digit-only choices (`1` through `7`, with `0` to exit); users do not need to type CLI commands inside the UI. The guided workflow selects a PAK from `PAK/`, unpacks it to `UNPACK/<name>/` on the first run, and repacks the edited workspace to `RESULT/<name>.pak` on the next run. Advanced CLI flags remain available for developers, while the normal `pakforge` and `tool` entry points open the same neon interface.
