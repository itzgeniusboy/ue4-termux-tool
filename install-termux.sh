...
if [ "${PAKTOOL_DEFER_SETUP:-0}" = "1" ]; then
  printf '%s\n' "[3/5] Deferring Paktool Python dependencies to background setup."
else
  printf '%s\n' "[3/5] Installing Paktool Python dependencies..."
...
PAKTOOL_ROOT_DIR="/sdcard/Paktool"
SDCARD_PAK_DIR="\$PAKTOOL_ROOT_DIR/PAK"
SDCARD_EDIT_DIR="\$PAKTOOL_ROOT_DIR/EDIT"
SDCARD_UNPACKED_DIR="\$PAKTOOL_ROOT_DIR/UNPACKED"
SDCARD_MODDED_DIR="\$PAKTOOL_ROOT_DIR/MODDED"
if mkdir -p "\$PAKTOOL_ROOT_DIR" "\$SDCARD_PAK_DIR" "\$SDCARD_EDIT_DIR" "\$SDCARD_UNPACKED_DIR" "\$SDCARD_MODDED_DIR"; then
  printf '%s\\n' "[Paktool] SD-card folders ready: \$PAKTOOL_ROOT_DIR, \$SDCARD_PAK_DIR, \$SDCARD_EDIT_DIR, \$SDCARD_UNPACKED_DIR, \$SDCARD_MODDED_DIR"
else
  printf '%s\\n' "[Paktool] SD-card folders could not be created. Run: termux-setup-storage" >&2
fi
...
if [ "\${PAKTOOL_NO_UPDATE:-0}" != "1" ] && [ -d "\$SCRIPT_DIR/.git" ]; then
  UPDATE_LOCK="\$STATE_DIR/update.lock"
  if mkdir "\$UPDATE_LOCK" 2>/dev/null; then
    printf '%s\n' "[Paktool] GitHub update check started in the background."
    (
      ...
      if [ -n "\$(git status --porcelain --untracked-files=all)" ]; then
        dirty=1
        stash_name="paktool-autoupdate-\$(date +%Y%m%d-%H%M%S)"
        if git stash push --include-untracked --message "\$stash_name" >/dev/null 2>&1; then
          printf '%s Paktool auto-stashed local changes as %s before update.\n' "\$now" "\$stash_name" >> "\$UPDATE_LOG"
        else
          printf '%s Paktool update failed: could not auto-stash local changes.\n' "\$now" >> "\$UPDATE_LOG"
      ...
      restore_stash() {
        if [ "\$dirty" = "1" ]; then
          if git stash pop --index >/dev/null 2>&1; then
            printf '%s Paktool restored local changes after update failure.\n' "\$now" >> "\$UPDATE_LOG"
          else
            printf '%s Paktool could not automatically restore local changes; use git stash list and git stash pop.\n' "\$now" >> "\$UPDATE_LOG"
          fi
        fi
      }
      if ! git fetch --quiet origin main; then
        restore_stash
        printf '%s Paktool update failed during fetch.\n' "\$now" >> "\$UPDATE_LOG"
      ...
      if git merge --ff-only --quiet origin/main; then
        ...
        if [ "\$before" = "\$after" ]; then
          ...
          SKIP_PACKAGES=1 PAKTOOL_DEFER_SETUP=1 bash "\$SCRIPT_DIR/install-termux.sh" >> "\$UPDATE_LOG" 2>&1 || true
        fi
      else
        ...
        if git branch "\$backup_branch" "\$before" >/dev/null 2>&1 && git reset --hard origin/main >/dev/null 2>&1; then
          printf '%s Paktool reset to origin/main; previous local commits are preserved in branch %s.\n' "\$now" "\$backup_branch" >> "\$UPDATE_LOG"
          if [ "\$dirty" = "1" ]; then
          ...
          restore_stash
          printf '%s Paktool update skipped: unable to synchronize origin/main safely.\n' "\$now" >> "\$UPDATE_LOG"
          printf '{"state":"skipped","time":"%s","message":"Local work was preserved; origin/main could not be synchronized safely."}\n' "\$now" > "\$UPDATE_STATUS"
        fi
      fi
    ) >/dev/null 2>&1 &
  else
    printf '%s\n' "[Paktool] GitHub update check is already running in the background."
  fi
fi

if [ "\${1:-}" = "setup-status" ]; then
  exec python3 "\$SCRIPT_DIR/paktool_setup.py" --status
fi

SETUP_LOG="\$STATE_DIR/setup.log"
SETUP_STATUS="\$STATE_DIR/setup-status.json"
if [ "\${PAKTOOL_NO_SETUP:-0}" != "1" ] && { [ ! -f "\$SETUP_STATUS" ] || ! grep -q '"state": "ready"' "\$SETUP_STATUS" 2>/dev/null || ! python3 -c 'import rich, pytz, gmalg, Crypto, zstandard' >/dev/null 2>&1 || ! command -v repak >/dev/null 2>&1; }; then
  if [ ! -d "\$STATE_DIR/setup.lock" ]; then
    printf '%s\n' "[Paktool] Setup is running in the background. Log: \$SETUP_LOG"
    nohup python3 "\$SCRIPT_DIR/paktool_setup.py" --background >>"\$SETUP_LOG" 2>&1 </dev/null &
  else
    printf '%s\n' "[Paktool] Background setup is already running. Log: \$SETUP_LOG"
  fi
fi

if ! python3 -c 'import rich, pytz, gmalg, Crypto, zstandard' >/dev/null 2>&1; then
  printf '%s\n' "[Paktool] Core dependencies are still installing in the background." >&2
  exec python3 "\$SCRIPT_DIR/paktool_first_run.py" --script "\$SCRIPT_DIR/paktool.py" "\$@"
fi
