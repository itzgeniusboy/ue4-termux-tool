...
The normal `tool` command performs a GitHub `origin/main` synchronization before opening PakForge, so one command installs the latest source and then launches the latest UI. Tracked and untracked local edits are temporarily saved in a timestamped Git stash, while divergent local commits are preserved in a timestamped backup branch before `origin/main` is installed. The launcher scripts are regenerated automatically when the source changes. Direct `pakforge` launches retain a non-blocking background update check. Update state and logs are available at `~/.local/state/pakforge/update-status.json` and `~/.local/state/pakforge/update.log`. Use `PAKFORGE_NO_UPDATE=1 tool` or `PAKFORGE_NO_UPDATE=1 pakforge` only when an update check must be disabled for one launch.
...
The `tool` command is the one-command launcher for the same neon PakForge UI. Each normal `tool` launch synchronizes the latest GitHub source before opening. If you need the old repak-based wrapper, use `UE4TOOL_LEGACY=1 tool ...`. If you use the manual installer path, run `pakforge` once after the launcher is created; afterward `tool` can refresh the source and open the latest version automatically.
