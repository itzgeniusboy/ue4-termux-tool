...
# Authorized project SM4 derivation secrets. Keep these values private and never
# print them in diagnostics or operation logs.
...
def repack_pak_file_full(pak_file, edited_root, output_path, target_path=None, force_add=False, workers=4, delete_paths=None):
    """
    ...
    normalized_delete_paths = {
        PurePosixPath(str(path).replace('\\', '/')).as_posix().lstrip('./').lower()
        for path in (delete_paths or set())
    }
    if not edit_files and not normalized_delete_paths:
        console.print('[bold red]❌ No files found in EDIT folder and no delete paths were supplied![/bold red]')
        return 0

    if edit_files:
        console.print(f'[bold cyan]📁 Found {len(edit_files)} files in EDIT folder[/bold cyan]')
    if normalized_delete_paths:
        console.print(f'[bold yellow]🗑️ Delete targets: {len(normalized_delete_paths)}[/bold yellow]')
    ...
    removed_count = 0
    if normalized_delete_paths:
        for directory, files in list(all_dirs.items()):
            for name, entry in list(files.items()):
                full_path = PurePosixPath(str(PurePath(directory) / name).replace('\\', '/')).as_posix().lstrip('./')
                if full_path.lower() in normalized_delete_paths:
                    del files[name]
                    removed_entry_ids.add(id(entry))
                    removed_count += 1
            if not files:
                del all_dirs[directory]
        if removed_count == 0:
            requested = ', '.join(sorted(normalized_delete_paths))
            raise ValueError(f'None of the requested PAK paths were found: {requested}')
        console.print(f'[bold yellow]🗑️ Removed {removed_count} existing PAK entrie(s)[/bold yellow]')
