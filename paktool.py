...
def _normalize_pak_path(value: str) -> str:
    """Normalize a user-supplied logical PAK path without allowing traversal."""
    raw = value.strip().replace("\\", "/")
    path = PurePosixPath(raw)
    if path.is_absolute() or ".." in path.parts:
        raise SystemExit(f"Unsafe PAK path: {value}")
    normalized = path.as_posix().lstrip("./")
    if not normalized or normalized == ".":
        raise SystemExit(f"Invalid empty PAK path: {value}")
    return normalized


def delete_command(args: argparse.Namespace) -> None:
    """Remove selected logical entries and write a newly verified PAK."""
    source_pak = require_file(args.source_pak, "Source PAK")
    output = Path(args.output).expanduser()
    refuse_existing(output, args.overwrite)
    requested = [_normalize_pak_path(value) for value in args.paths]
    if len({item.lower() for item in requested}) != len(requested):
        raise SystemExit("Duplicate delete paths were supplied.")

    pak, is_od, _ = open_pak_auto(source_pak, args.is_od)
    available = {row["path"].replace("\\", "/"): row["path"] for row in inventory(pak)}
    lower_available = {key.lower(): value for key, value in available.items()}
    resolved: list[str] = []
    for value in requested:
        exact = lower_available.get(value.lower())
        if exact is None:
            suffix_matches = [path for path in available.values() if path.lower().endswith("/" + value.lower())]
            if len(suffix_matches) == 1:
                exact = suffix_matches[0]
            elif len(suffix_matches) > 1:
                raise SystemExit(f"Ambiguous PAK filename '{value}'. Use its full logical path.")
        if exact is None:
            raise SystemExit(f"PAK entry not found: {value}")
        resolved.append(exact)

    output.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="paktool-delete-") as temp_name:
        empty_edit_dir = Path(temp_name) / "empty-edit"
        empty_edit_dir.mkdir()
        count = repack_pak_file_full(
            pak,
            empty_edit_dir,
            output,
    ...
    if count <= 0:
        raise SystemExit("No PAK entries were deleted.")

    verified, verified_od, _ = open_pak_auto(output, args.is_od)
    remaining = {row["path"].lower() for row in inventory(verified)}
    still_present = [path for path in resolved if path.lower() in remaining]
    if still_present:
        output.unlink(missing_ok=True)
        raise SystemExit("Verification failed; entries still present: " + ", ".join(still_present))
    parser_mode = "od" if verified_od else "standard"
    print(f"Deleted {len(resolved)} PAK entrie(s).")
    print(f"Verified output: {output} ({parser_mode} parser)")
...
def parser() -> argparse.ArgumentParser:
    ...
    delete = sub.add_parser("delete", help="remove selected logical entries and create a new PAK")
    delete.add_argument("source_pak", help="source UE4 PAK")
    delete.add_argument("paths", nargs="+", help="logical PAK paths; a unique basename is also accepted")
    delete.add_argument("output", help="new output PAK path")
    delete.add_argument("--overwrite", action="store_true")
    delete.add_argument("--workers", type=int, default=4, help="parallel staging workers (default: 4)")
